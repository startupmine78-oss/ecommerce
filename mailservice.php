...
class MailService
{
    ...
    //Имэйл Явуулах (PHPMailer)
    public static function sendMail(string $to, string $subject, string $html): array
    {
        // PHPMailer байхгүй бол PHP mail() ашиглах
        if (!PHPMAILER_AVAILABLE) {
            return self::sendMailFallback($to, $subject, $html);
        }
        ...
        try {
            ...
            return ['success' => false, 'error' => 'Имэйл илгээхэд алдаа гарлаа: ' . $e->getMessage()];
        }
    }

    //Fallback: PHP mail()
    private static function sendMailFallback(string $to, string $subject, string $html): array
    {
        ...
        return $result
            ? ['success' => true]
            : ['success' => false, 'error' => 'mail() амжилтгүй. SMTP тохируулна уу.'];
    }
